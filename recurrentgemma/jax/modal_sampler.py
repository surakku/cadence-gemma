...
class ModalSampler(Generic[Cache]):
  ...
  def apply_model(
      ...
      return_logits: bool,
      return_cache: bool,
      img_embed: at.Image | None = None
  ) -> tuple[at.TokenLogits | None, Cache | None]:
    # if img_embed:
    #   img_embed = img_embed[None, :, :]
  ...
  def __call__(
    ...
    if img_path != "":
        img_emb = torch.squeeze(self.vis_enc(img_path))
        img_emb = t2j(img_emb).astype(jnp.bfloat16)
